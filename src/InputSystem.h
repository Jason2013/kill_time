...
public:
  ...
  bool KeyPressing(unsigned char key)
  {
	  bool state = KeyDown(key);
	  if (state != m_lastState[key])
	  {
		  m_lastState[key] = state;
		  if (state)
			  return true;
	  }
	  return false;
  ...
  void  Clear() { ZeroMemory(m_keys, 256 * sizeof(char)); memset(m_lastState, 0, sizeof(m_lastState)); }
...
public:
  ...
  bool  KeyPressing(unsigned char key) { return (m_pKeyboard && m_pKeyboard->KeyPressing(key)); }
